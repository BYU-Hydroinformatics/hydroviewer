...
def get_warning_points(request, app_workspace):
    ...
    canada_id_path = os.path.join(app_workspace.path, 'canada_reachids.csv')
    reach_pds = pd.read_csv(canada_id_path)
...
def get_discharge_data(request):
    ...
    try:
        ...
        url = 'https://dd.weather.gc.ca/hydrometric/csv/{0}/daily/{0}_{1}_daily_hydrometric.csv'.format(province, codEstacion)

        df = pd.read_csv(url)
        df.drop(columns=[' ID', 'Grade', 'Symbol / Symbole', 'QA/QC', 'Grade.1', 'Symbol / Symbole.1', 'QA/QC.1'], inplace=True)
        df.drop(columns=["Water Level / Niveau d'eau (m)"], inplace=True)
        df.rename(columns={"Discharge / Débit (cms)": "Streamflow (m3/s)"}, inplace=True)
...
def get_observed_discharge_csv(request):
    ...
    try:
        ...
        url = 'https://dd.weather.gc.ca/hydrometric/csv/{0}/daily/{0}_{1}_daily_hydrometric.csv'.format(province,
                                                                                                        codEstacion)

        df = pd.read_csv(url)
        df.drop(columns=[' ID', 'Grade', 'Symbol / Symbole', 'QA/QC', 'Grade.1', 'Symbol / Symbole.1', 'QA/QC.1'],
                inplace=True)
        df.drop(columns=["Water Level / Niveau d'eau (m)"], inplace=True)
        df.rename(columns={"Discharge / Débit (cms)": "Streamflow (m3/s)"}, inplace=True)
        df.rename(columns={"Date": "Datetime"}, inplace=True)
        ...
        datesObservedDischarge = observed_rt_plot.index
        observedDischarge = observed_rt_plot.iloc[:, 0].values
...
def get_observed_waterlevel_csv(request):
    ...
    try:
        ...
        datesObservedWaterLevel = observed_rt_plot.index
        observedWaterLevel = observed_rt_plot.iloc[:, 0].values
