...
def get_geoserver_endpoint():
    ...
    geoserver_endpoint = TextInput(display_text='',
                                   initial=json.dumps(
                                       [geoserver_base_url, geoserver_workspace, region, extra_feature]),
...
def get_zoom_info():
    return TextInput(display_text='',
                     initial=json.dumps(
                         app.get_custom_setting('zoom_info')),
                     name='zoom_info',
                     disabled=True)


def get_model_input(init_model_val=None):
    return SelectInput(display_text='',
                       name='model',
                       multiple=False,
...
# We use only one controller now since LIS data has been updated to come from an api from the BLDAS explorer


def lis_ecmwf(request):
    ...
    default_model = app.get_custom_setting('default_model_type')
    init_model_val = request.GET.get(
        'model', False) or default_model or 'Select Model'
    init_ws_val = app.get_custom_setting(
        'default_watershed_name') or 'Select Watershed'
    ...
    res2 = requests.get(app.get_custom_setting('geoserver') + '/rest/workspaces/' +
                        app.get_custom_setting('workspace') + '/featuretypes.json', verify=False)

    for i in range(len(json.loads(res2.content)['featureTypes']['featureType'])):
        raw_feature = json.loads(res2.content)[
            'featureTypes']['featureType'][i]['name']
        if 'drainage_line' in raw_feature and any(n in raw_feature for n in app.get_custom_setting('keywords').replace(' ', '').split(',')):
            ...
            if feat_name not in str(watershed_list):
                watershed_list.append([feat_name, raw_feature])
    ...
    watershed_select = SelectInput(display_text='',
                                   ...
                                   classes=hiddenAttr,
                                   attributes={
                                       'onchange': "javascript:view_watershed();"+hiddenAttr}
                                   )

    context = {
        "base_name": base_name,
        "model_input": get_model_input(init_model_val),
    ...
    if (init_model_val == 'ECMWF-RAPID'):
        ...
        renderer = '{0}/lis.html'.format(base_name)

    return render(request, renderer, context)
...
def hiwat(request):
    ...
    watershed_select = SelectInput(display_text='',
                                   ...
                                   original=True,
                                   attributes={
                                       'onchange': "javascript:view_watershed();"}
                                   )
...
def get_historic_data(request):
    if request.GET['model'] == 'ECMWF-RAPID':
        return ecmwf_get_historic_data(request)
    elif request.GET['model'] == 'LIS-RAPID':
        return lis_get_historic_data(request)
    else:
        return JsonResponse({'error': 'No data found for the selected model.'})

# The two functions below could possibly be merged into one. Refactor needed there.


def lis_get_historic_data(request):
    get_data = request.GET
    try:
        watershed = get_data['watershed']
        subbasin = get_data['subbasin']
        ...
        units = 'metric'
        res = requests.get(
            app.get_custom_setting('sldas_source') + '/apps/bldas-explorer/api/GetForecast?watershed_name=' +
        ...
        for pair in pairs:
            dates.append(dt.datetime.strptime(
                pair.split(',')[0], '%Y-%m-%d %H:%M:%S'))
            mean_values.append(float(pair.split(',')[1]))
        # ----------------------------------------------
        # Chart Section
        # ----------------------------------------------
        ...
        layout = go.Layout(
            title="Historical Data<br><sub>{0} ({1}): {2}</sub>".format(
                watershed, subbasin, comid),
            xaxis=dict(
                title='Date',
                ),
    ...
    for date in eval(res.content):
        if len(date) == 10:
            ...
            date_f = dt.datetime.strptime(
                date_mod, '%Y%m%d.%H%M').strftime('%Y-%m-%d %H:%M')
        else:
            date_f = dt.datetime.strptime(
...
def get_lis_data_csv(request):
    ...
    try:
        ...
        path = os.path.join(app.get_custom_setting(
            'lis_path'), '-'.join([watershed, subbasin]))
        filename = [f for f in os.listdir(path) if 'Qout' in f]
        res = nc.Dataset(os.path.join(app.get_custom_setting(
            'lis_path'), '-'.join([watershed, subbasin]), filename[0]), 'r')
    ...
    try:
        ...
        if not os.path.exists(reprojected_shp_path):
            ...
            while in_feature:
                ...
                out_feature.SetField('COMID', in_feature.GetField(
                    in_feature.GetFieldIndex('COMID')))
...
# res = requests.get(app.get_custom_setting('api_source') + '/apps/streamflow-prediction-tool/api/GetWatersheds/',
    #                    headers={'Authorization': 'Token ' + app.get_custom_setting('spt_token')})
        ...
        request_headers = dict(Authorization='Token ' +
                               app.get_custom_setting('spt_token'))
