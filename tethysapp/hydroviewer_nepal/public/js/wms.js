    ...
    if (JSON.parse($('#geoserver_endpoint').val())[3]) {
        var wmsLayer = new ol.layer.Image({
            source: new ol.source.ImageWMS({
                ...
                params: {
                    LAYERS: JSON.parse($('#geoserver_endpoint').val())[3]
                },
        ...
        layers = [
            ...
            .concat(wms_layers)
            .concat([wmsLayer, featureOverlay]);
    } else {
        layers = [
            base_layer,
            two_year_warning,
...
function view_watershed() {
    ...
    let isECMWF = $('#model option:selected').text() === 'ECMWF-RAPID',
        isLIS = $('#model option:selected').text() === 'LIS-RAPID';
    ...
    if (
        ...
        let workspace = JSON.parse($('#geoserver_endpoint').val())[1];
        let model = $('#model option:selected').text();
        let watershed = $('#watershedSelect option:selected').val();

        let subbasin = $('#watershedSelect option:selected')
            .text()
            .split(' (')[1]
            .replace(')', '')
            .toLowerCase();
        let watershed_display_name = $('#watershedSelect option:selected')
            .text()
            .split(' (')[0];
        let subbasin_display_name = $('#watershedSelect option:selected')
            .text()
            .split(' (')[1]
            .replace(')', '');
        ...
        var layerName = workspace + ':' + watershed;
        ...
        if (isECMWF) get_warning_points(model, watershed, subbasin);
        ...
        $('#model option:selected').text() === 'HIWAT-RAPID' &&
        $('#watershedSelect option:selected').val() !== ''
    ) {
        $('#watershed-info').empty();
...
function map_events() {
    map.on('pointermove', function(evt) {
        ...
        if (model === 'ECMWF-RAPID' || model === 'LIS-RAPID') {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            if (
                                ...
                                let split = JSON.parse(
                                    $('#geoserver_endpoint').val()
                                )[2].split('-');

                                var watershed = split[0];
                                var subbasin = split[1];
                            } else {
                                var watershed = result['features'][0][
                                    'properties'
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            if (
                                ...
                                var watershed = dFrSplit[0];
                                var subbasin = dFrSplit[1];
                            ...
                            $('#info').addClass('hidden');
                            // Dunno what this add_feature does yet. Commenting out. Need to tackle it
                            // add_feature(model, workspace, comid);
                        },
                        error: function(error) {
                            console.log(error);
...
function submit_model() {
    $('#model').on('change', function() {
        ...
        if (
            ...
            base_path.includes('hiwat-rapid')
        ) {
            base_path = base_path
                .replace('/ecmwf-rapid', '')
                .replace('/lis-rapid', '')
...
$(function() {
    ...
    window.onresize = function() {
        $('#graph .modal-body .tab-pane.active .js-plotly-plot').each(
            function() {
